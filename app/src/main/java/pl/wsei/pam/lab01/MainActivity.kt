...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var testsList = mutableListOf<Boolean>();
    ...
    fun task15(degree: String): Int {
        var dict = mapOf("bardzo dobry" to 5, "dobry" to 4,"dostateczny" to 3,"dopuszczający" to 2,
            "niedostateczny" to 1);
        var input = degree.lowercase();
        return dict.getOrDefault(input,-1);
    }
    ...
    fun task16(store: Map<String, UInt>, asset: Map<String, UInt>): UInt {
        var list = mutableListOf<UInt>();
        asset.forEach{entry ->
            val value = store[entry.key]
            if (value != null) {
                var counter = value / entry.value
                list.add(counter);
            }
        }
        return list.minOrNull() ?: 0u
    }
}